...
**3. ขั้นตอนก่อนนำ SDCard ไปทำไฟล์ Image backup :**

```bash
sudo visudo
```
เลื่อนเคอร์เซอร์ลงมาที่ บรรทัดล่างสุด ของไฟล์ แล้วเพิ่มข้อความนี้ลงไป (หากคุณใช้ user อื่นที่ไม่ใช่ mumble ให้เปลี่ยนชื่อให้ตรงด้วยครับ):


```Plaintext
mumble ALL=(ALL) NOPASSWD: ALL
```
กด Ctrl + O -> Enter เพื่อเซฟ และกด Ctrl + X เพื่อออก

**2. ขั้นตอนที่ 2: ล้างรหัสประจำเครื่องก่อนทำ Image (Sysprep)
แทนที่จะให้ผู้ใช้ต้องมาแก้ Machine ID เอง เราสามารถ "ล้างรหัส" ทิ้งไว้ตั้งแต่ตอนเป็นบอร์ดต้นฉบับ พอผู้ใช้เอาไปเสียบเปิดเครื่องครั้งแรก ระบบ Linux จะสร้างรหัสใหม่ให้เองโดยอัตโนมัติครับ

เมื่อคุณตั้งค่าทุกอย่างเสร็จแล้ว และ "เตรียมจะปิดเครื่องเพื่อถอด SD Card ไปทำ Image" ให้พิมพ์ 3 คำสั่งนี้เป็นสิ่งสุดท้ายครับ::
...
* คำสั่งที่ 1 และ 2 จะทำหน้าที่ล้างรหัสประจำเครื่องทิ้ง (เปลี่ยนให้เป็นไฟล์ว่างๆ)
* คำสั่งที่ 3 จะปิดเครื่องทันที
* เมื่อไฟสีเขียวดับสนิท ให้ถอด SD Card ออกไปทำ Image (Clone) ได้เลยครับ
เพียงเท่านี้ Image ตัวใหม่ของคุณก็จะเป็น "Master Image" ที่สมบูรณ์แบบ ไม่ว่าจะถูกนำไปเขียนลง SD Card อีกกี่สิบใบ เมื่อเปิดเครื่องขึ้นมา มันจะสร้าง ID ใหม่ของตัวเองอัตโนมัติ และปุ่ม Shutdown/Restart บน WebUI จะกดใช้งานได้ทันทีโดยไม่ต้องเข้า SSH เลยครับ!
