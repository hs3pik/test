# 📖 คู่มือการสร้าง Mumble Radio Gateway บน Raspberry Pi (พร้อม WebUI และ TOT Watchdog)
...
💡 **ทริคก่อนเริ่มทำ:**
ในคู่มือนี้จะมีการอ้างอิงถึงตำแหน่งไฟล์ หากไม่แน่ใจว่าเครื่อง Raspberry Pi ของคุณใช้ชื่อ User ว่าอะไร ให้พิมพ์คำสั่ง `whoami` ใน Terminal 
ให้นำชื่อที่ได้ไปแทนที่คำว่า `[ชื่อUSER]` ในขั้นตอนการตั้งค่า Service ทุกจุดนะครับ

## บทที่ 1: อุปกรณ์ที่ต้องเตรียม (Hardware & Components)
* **คอมพิวเตอร์บอร์ดเดี่ยว:** Raspberry Pi 2, 3 หรือ 4 พร้อม MicroSD Card และอะแดปเตอร์
...
แนะนำให้ใช้ Raspberry Pi OS Lite หรือเวอร์ชัน Bookworm ขึ้นไป
...
```text
[ โมดูล I2C ด้านหลังจอ LCD ]                 [ ขา PIN บน Raspberry Pi ]

     (VCC) -----------------------------------> [Pin 2]  หรือ [Pin 4] (ไฟ 5V)
     (GND) -----------------------------------> [Pin 6]  (Ground)
     (SDA) -----------------------------------> [Pin 3]  (GPIO 2)
     (SCL) -----------------------------------> [Pin 5]  (GPIO 3)
```
...
```text
[ ปุ่มกดที่ 1 : UP (เลื่อนขึ้น) ]
   ขาด้านที่ 1 -------------------------------> [Pin 11] (GPIO 17)
   ขาด้านที่ 2 -------------------------------> [Pin  9] (Ground)

[ ปุ่มกดที่ 2 : DOWN (เลื่อนลง) ]
   ขาด้านที่ 1 -------------------------------> [Pin 13] (GPIO 27)
   ขาด้านที่ 2 -------------------------------> [Pin 14] (Ground)
```
...
* **เรื่อง USB Sound Card:** โค้ดที่ออกแบบมานี้ ตั้งค่าเริ่มต้นให้ค้นหา Sound Card ชิป `CM108 (C-Media)` หากคุณใช้ Sound Card ยี่ห้อ/รุ่นอื่น คุณจะต้องเข้าไปตรวจสอบชื่อ (ด้วยคำสั่ง `lsusb` หรือ `aplay -l`) แล้วนำชื่อนั้นมาแก้แทนคำว่า "C-Media" ในโค้ด (บรรทัดที่ 113)
* **ตัวอย่างการแก้ไข:** สมมติว่า Sound Card ตัวใหม่ของคุณชื่อ "Ugreen" ให้แก้บรรทัดค้นหาเสียงเป็น: `if "USB" in dev_info['name'] or "Ugreen" in dev_info['name']:` (เปลี่ยนแทนคำหลังคำเดียวได้เลย)

💡 **เกร็ดความรู้:** จริงๆ แล้ว ถ้า Sound Card ตัวใหม่เป็นแบบเสียบพอร์ต USB ส่วนใหญ่มักจะมีคำว่า "USB" แฝงในชื่ออยู่แล้ว ซึ่งถ้าเป็นแบบนั้น ระบบจะเข้าเงื่อนไขคำว่า "USB" ตัวหน้าให้โดยอัตโนมัติ ทำให้ทำงานได้เลยโดยที่คุณไม่ต้องแก้โค้ดแม้แต่ตัวเดียวครับ! แต่ถ้าอยากให้ชัวร์ 100% ก็เอาชื่อยี่ห้อใหม่ไปใส่แทน "C-Media" ได้เลยครับ
...
VOX_THRESHOLD = 400    
VOX_HANG_TIME = 0.8    # ค่าหน่วงฝั่งรับเสียง (RX)
TX_HANG_TIME = 0.7     # ค่าหน่วงฝั่งกดคีย์ (TX)
...
**วิธีเข้าหน้า WebUI:**
ให้เปิดเบราว์เซอร์ในมือถือหรือคอมพิวเตอร์ที่ต่อเน็ต/Wi-Fi วงเดียวกัน แล้วพิมพ์ URL ว่า `http://[IP-ของ-Pi]:8080` (ดูเลข IP ได้ที่หน้าจอ LCD ของตัวเครื่องเลยครับ)
...
**2. ใส่โค้ดตั้งค่า (🔴 อย่าลืมเปลี่ยน `[ชื่อUSER]` เป็นชื่อ User ของคุณ):**
```ini
[Unit]
Description=Mumble Radio Gateway
After=network.target sound.target

[Service]
ExecStart=/usr/bin/python3 /home/[ชื่อUSER]/gateway.py
WorkingDirectory=/home/[ชื่อUSER]/
...
def read_gpio_state(pin):
    try:
        # ใช้คำสั่ง pinctrl เพื่อเช็คไฟที่ขา GPIO แบบไม่ชนกับโปรแกรมหลัก
        result = subprocess.check_output(['pinctrl', 'get', str(pin)], text=True)
        if '| hi' in result:
            return True
        return False
    except Exception as e:
...
ใส่โค้ดตั้งค่าดังนี้ (🔴 อย่าลืมเปลี่ยน `[ชื่อUSER]` เป็นชื่อ User ของคุณ):
```ini
[Unit]
...
[Service]
ExecStart=/usr/bin/python3 /home/[ชื่อUSER]/tot_watchdog.py
WorkingDirectory=/home/[ชื่อUSER]/
StandardOutput=inherit
StandardError=inherit
Restart=always
